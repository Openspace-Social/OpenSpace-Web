        ...
        <ok-user-tile :user="user" clickable>
            <template v-slot:trailing>
                <ok-block-button
                    ...
                    :user="user" :buttonSize="OkFollowButtonSize.small"></ok-block-button>
            </template>
        </ok-user-tile>
    </article>
</template>
...
<script lang="ts">
    ...
    @Component({
        name: 'OkBlockedUserListItem',
        components: {OkBlockButton, OkUserTile }
    })
    export default class OkBlockedUserListItem extends Vue {
        @Prop({
            type: Object,
            required: true
        }) readonly user: IUser;
